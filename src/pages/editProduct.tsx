...
const EditProduct = () => {
  ...
  const { imageUrls,  uploadSingleFile, removeImage, isPending } = useUploadImages()
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
       ...
       setOriginalImages(product.imageUrl || [])
      } catch (err: any) {
 ...
 const handleSubmit = async (values: any) => {
    try {
      ...
      // 👇 if no new images, fallback to original ones
      const productData = { 
        ...values, 
        imageUrl: imageUrls.length > 0 ? imageUrls : originalImages 
      }
  ...
  if (!initialValues) return <p>Loading product...</p>
      ...
      >
        {() => (
          <Form className="space-y-4">
            ...
            >
              {submitting || isPending ? 'Updating...' : 'Update Product'}
            </button>
          </Form>
        )}
      </Formik>
